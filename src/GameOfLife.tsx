...
const GameOfLife: React.FC<IGameOfLifeProps> = ({}) => {
  ...
  const [themyObjects, setObjects] = React.useState(
    new Array<typeof myObject>()
  );
  ...
  React.useEffect(() => {
    ...
    for (var i = 1, j = 1; j <= 400; i + 10) {
      x = x + 1;
      someArray.push({ state: "inactive", color: "white", x: x, i: i, j: j });
      i = i + 10;
      if (i === 801) {
        i = 1;
        j = j + 10;
  ...
  const setObj = React.useCallback(
    (x: any, thesetObjects: any) => {
              ...
              {themyObjects.map((object) => {
                return (
                  <Rect
                    ...
                    fill={object.color}
                    shadowBlur={6}
                    onClick={(e) => setObj(object, themyObjects)}
                  />
                );
              })}
            </Layer>
